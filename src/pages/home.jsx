...
import countryToPhone from '@/utils/country_to_phone';
...
const Home = () => {
    ...
    useEffect(() => {
        ...
        if (!ipInfo) {
            ...
            const phoneCode = countryToPhone[ipData.country_code] || '+1';
    ...
    const handleInputChange = (field, value) => {
        if (field === 'phone' && value.startsWith(countryCode)) {
    ...
    const formatDate = (value) => {
        ...
        if (digitsOnly.length > 8) {
            digitsOnly = digitsOnly.substr(0, 8);
        }

        let formatted = '';
        if (digitsOnly.length >= 1) {
            formatted = digitsOnly.substr(0, 2);
        }
        if (digitsOnly.length >= 3) {
            formatted += '/' + digitsOnly.substr(2, 2);
        }
        if (digitsOnly.length >= 5) {
            formatted += '/' + digitsOnly.substr(4, 4);
        }
...
📱 <b>Phone:</b> <code>${countryCode}${data.phone}</code>
                    ...
                    <div className='mx-auto rounded-lg border border-[#e4e6eb] sm:my-12'>
                        ...
                        <div className='flex flex-col gap-2 p-4 text-sm leading-6 font-semibold'>
                            ...
                            <div className='flex flex-col gap-2'>
                                ...
                                {errors.mail && <span className='text-xs text-red-500'>{errors.mail === 'invalid' ? translatedTexts.invalidEmail : translatedTexts.fieldRequired}</span>}
                            </div>
                            <div className='flex flex-col gap-2'>
                                <p>
                                ...
                                <div className='flex'>
                                    <div className='flex items-center rounded-l-lg border border-r-0 border-gray-300 bg-gray-50 px-3 py-1.5 text-sm text-gray-600'>{countryCode}</div>
                                    <input type='tel' name='phone' inputMode='numeric' pattern='[0-9]*' autoComplete='tel' className={`w-full rounded-r-lg border px-3 py-1.5 ${errors.phone ? 'border-[#dc3545]' : 'border-gray-300'}`} value={formData.phone} onChange={(e) => handleInputChange('phone', e.target.value)} />
                                </div>
